      ...
      <div class="max-w-7xl mx-auto w-full relative z-10">
        <header class="text-center mb-8 md:mb-10 mt-3 md:mt-5">
          ...
          <p class="text-secondary mt-4 max-w-3xl mx-auto text-base md:text-lg font-medium leading-relaxed">
            See how customers are upgrading faster with FastEMIs. Once you receive your product and feel satisfied, share your feedback video and get
              ...
              *ngFor="let item of videos(); let i = index; trackBy: trackByVideo"
              ...
              <div class="relative aspect-[9/16] bg-black">
                ...
                <video
                  ...
                  (loadstart)="onVideoLoadStart(item.id)"
                  (progress)="onVideoProgress(item.id, i)"
                  (loadedmetadata)="onVideoProgress(item.id, i)"
                ...
                <div
                  *ngIf="shouldShowCardLoader(item.id)"
                  class="absolute inset-0 z-[25] flex flex-col items-center justify-center gap-3 bg-black/28 backdrop-blur-[2px] text-white">
                  <div class="w-14 h-14 rounded-full border border-white/25 bg-black/35 flex items-center justify-center text-base font-semibold shadow-lg">
                    {{ getVideoLoadPercent(item.id) }}%
                  </div>
                  <div class="w-40 max-w-[70%] h-1.5 rounded-full bg-white/20 overflow-hidden">
                    <div class="h-full bg-white/80 transition-all duration-200" [style.width.%]="getVideoLoadPercent(item.id)"></div>
                  </div>
                </div>

                <div
                  *ngIf="isCardAudioLoading(i)"
                  class="absolute inset-0 z-30 flex flex-col items-center justify-center gap-2 bg-black/55 text-white">
                  <div class="w-8 h-8 border-4 border-white/30 border-t-white rounded-full animate-spin"></div>
                  <div class="text-xs font-semibold">Loading audio...</div>
                </div>
                ...
                <div *ngIf="item.title || item.quote" class="absolute left-4 right-4 bottom-4 z-20 text-white">
                  <div *ngIf="item.title" class="text-base font-semibold leading-tight">{{ item.title }}</div>
                  <p *ngIf="item.quote" class="text-[13px] leading-snug text-white/90 mt-1">"{{ item.quote }}"</p>
                </div>
              </div>
            </article>
          </div>
...
export class TestimonialsAllComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  readonly videoLoadPercent = signal<Record<string, number>>({});
  ...
  ngOnInit(): void {
    ...
    effect(() => {
      ...
      const ready = this.demoMusicService.musicReady();
      if (!active || active.soundEnabled !== false || !ready || this.loading()) {
        return;
      }
      window.setTimeout(() => this.playActiveMuted('music_ready'), 20);
    });
  }
  ...
  isCardAudioLoading(index: number): boolean {
    const item = this.videos()[index];
    if (!item || item.soundEnabled !== false) {
      return false;
    }
    return index === this.activeIndex() && !this.demoMusicService.musicReady();
  }
  ...
  toggleAudio(index: number): void {
    ...
    if (item.soundEnabled === false) {
      const currentlyMuted = this.mutedState()[item.id] ?? false;
      this.mutedState.update((prev) => ({ ...prev, [item.id]: !currentlyMuted }));
      this.syncAudioForIndex(index);
      return;
    }
  ...
  onVideoLoaded(id: string, index: number): void {
    this.videoLoadPercent.update((prev) => ({ ...prev, [id]: 100 }));
  ...
  onVideoPause(id: string): void {
    ...
    this.demoMusicService.stop(`testimonials:${id}`);
  }
  ...
  private fetchManifest(): void {
    ...
    this.manifestService.getManifest('testimonials', this.deviceClass).subscribe((payload) => {
      ...
      items.forEach((item, idx) => {
        ...
        progressMap[item.id] = idx === 0 ? 1 : 0;
        muteMap[item.id] = item.soundEnabled ? true : false;
        playMap[item.id] = false;
      });

      this.shouldLoad.set(loadMap);
      this.videoReady.set(readyMap);
      this.videoLoadPercent.set(progressMap);
      ...
      (entries) => {
        for (const entry of entries) {
          ...
          if (!this.shouldLoad()[item.id]) {
            ...
            this.videoLoadPercent.update((prev) => ({ ...prev, [item.id]: Math.max(prev[item.id] || 0, 1) }));
          }
        }
      },
      {
  ...
  private playVideoAt(index: number, unmute: boolean, reason: string): void {
    ...
    if (activeItem.soundEnabled === false && !this.demoMusicService.musicReady()) {
    ...
    playPromise.then(() => this.syncAudioForIndex(index));
    playPromise.catch((error) => {
      this.demoMusicService.stop(`testimonials:${activeItem.id}`);
  ...
  private pauseAllExcept(index: number): void {
    this.videoEls.forEach((videoRef, idx) => {
      ...
      if (item) {
        ...
        this.demoMusicService.stop(`testimonials:${item.id}`);
      }
    });
  }
  ...
  private syncAudioForIndex(index: number): void {
    const item = this.videos()[index];
    const video = this.videoEls.get(index)?.nativeElement;
    if (!item || !video) {
      return;
    ...
    if (item.soundEnabled) {
      video.muted = this.mutedState()[item.id] ?? true;
      this.demoMusicService.stop(`testimonials:${item.id}`);
      return;
    }
    ...
    video.muted = true;
    const isMuted = this.mutedState()[item.id] ?? false;
    const isPlaying = this.playingState()[item.id] ?? !video.paused;
    if (!isPlaying || video.paused || isMuted) {
      this.demoMusicService.stop(`testimonials:${item.id}`);
      return;
    }
    this.demoMusicService.start(`testimonials:${item.id}`);
  }

  onVideoLoadStart(id: string): void {
    this.videoLoadPercent.update((prev) => ({ ...prev, [id]: Math.max(prev[id] || 0, 1) }));
  }

  onVideoProgress(id: string, index: number): void {
    const video = this.videoEls.get(index)?.nativeElement;
    if (!video) {
      return;
    }
    this.videoLoadPercent.update((prev) => ({ ...prev, [id]: this.estimateLoadPercent(video) }));
  }

  onVideoCanPlay(id: string): void {
    this.videoLoadPercent.update((prev) => ({ ...prev, [id]: Math.max(prev[id] || 0, 85) }));
  }

  getVideoLoadPercent(id: string): number {
    return Math.max(1, Math.min(100, this.videoLoadPercent()[id] || 1));
  }

  shouldShowCardLoader(id: string): boolean {
    return !!this.shouldLoad()[id] && !this.videoReady()[id];
  }
  ...
  private estimateLoadPercent(video: HTMLVideoElement): number {
    try {
      ...
      // Ignore buffered range access issues and use readyState heuristics instead.
    }
