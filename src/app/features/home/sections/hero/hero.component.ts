...
import { DemoMusicService } from '../../../../core/services/demo-music.service';
      ...
      <div class="container relative z-10 flex flex-col md:flex-row items-center gap-8 md:gap-16">
        <div class="flex-1 flex flex-col items-center text-center md:items-start md:text-left fade-in">
          <h1 class="font-display text-4xl md:text-5xl lg:text-6xl text-primary leading-tight mb-4 tracking-tight">
            Premium Gadgets In Easy EMI.<br/>
            <span class="text-accent relative inline-block typing-line">
              {{ typedHeroLine() }}<span class="typing-caret">|</span>
            </span>
          </h1>

          <p class="text-secondary text-base md:text-lg max-w-lg mb-8">
            Buy latest iPhone, laptop, or gaming console with monthly part payment. No cost EMI up to 12 months. No credit card needed. Student age 18 plus and low income users can apply. If you already have 3 or more active loans, you are not eligible.
          </p>
          ...
          <app-button variant="primary" [fullWidth]="false" class="w-full md:w-auto shadow-md" (onClick)="scrollToPartners()">
            Check EMI Options Now
          </app-button>

          <div class="mt-4 text-sm md:text-[15px] text-secondary">
            Need help now?
            <a href="mailto:support@fastemis.com" class="ml-1 font-medium text-primary no-underline hover:text-accent">
        ...
        <div class="flex-1 w-full max-w-sm md:max-w-md">
          <div class="hero-story-card">
            <div class="px-4 md:px-5 py-3 border-b border-white/60 bg-white/70 flex items-center justify-between gap-3">
              <div class="text-xs font-semibold text-primary/90 tracking-wide uppercase">Real Buyer Stories</div>
            ...
            <div
              ...
              <div *ngIf="loading() || isFallbackMusicLoading()" class="absolute inset-0 z-20 flex flex-col items-center justify-center gap-3 bg-[#081a2b] text-white">
                <div class="w-10 h-10 border-4 border-white/30 border-t-white rounded-full animate-spin"></div>
                <div class="text-sm font-semibold">{{ loading() ? 'Loading buyer stories' : 'Loading audio...' }}</div>
                <div *ngIf="!loading()" class="text-xs font-medium text-white/80">{{ activeVideoLoadPercent() }}%</div>
              </div>
              ...
              <ng-container *ngIf="!loading() && activeVideo() as current">
                ...
                <video
                  ...
                  (loadedmetadata)="onVideoProgress()"
                ...
                <div
                  *ngIf="waitingForFirstFrame()"
                  class="absolute inset-0 z-30 flex flex-col items-center justify-center gap-3 bg-black/25 backdrop-blur-[2px] text-white">
                  <div class="w-14 h-14 rounded-full border border-white/25 bg-black/35 flex items-center justify-center text-base font-semibold shadow-lg">
                    {{ activeVideoLoadPercent() }}%
                  </div>
                  <div class="w-44 max-w-[70%] h-1.5 rounded-full bg-white/20 overflow-hidden">
                    <div class="hero-loader-bar h-full transition-all duration-200" [style.width.%]="activeVideoLoadPercent()"></div>
                  </div>
                </div>

                <div *ngIf="current.title || current.quote" class="absolute inset-x-0 bottom-0 z-30 p-4 md:p-5 text-white">
                  <div *ngIf="current.title" class="text-lg md:text-xl font-semibold leading-tight">{{ current.title }}</div>
                  <div *ngIf="current.title" class="text-[11px] uppercase tracking-widest text-white/75 mt-1">Verified Customer</div>
                  <p *ngIf="current.quote" class="mt-2 text-sm md:text-base text-white/95 leading-relaxed">"{{ current.quote }}"</p>
                </div>
                ...
                <div class="absolute inset-x-0 top-0 z-40 px-4 pt-3">
                  <div class="h-1.5 w-full rounded-full bg-white/25 overflow-hidden">
                    <div class="hero-loader-bar h-full transition-all duration-300" [style.width.%]="activeVideoLoadPercent()"></div>
                  </div>
                </div>
              </ng-container>
            ...
            <div class="px-4 py-3 text-xs text-secondary bg-white/80 border-t border-white/60 flex items-center justify-between">
              <span>{{ isMobile() ? 'Swipe for next story' : 'Use controls for next story' }}</span>
...
export class HomeHeroComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  readonly activeVideoLoadPercent = signal<number>(1);
  readonly isMobile = signal<boolean>(false);
  readonly manifestVersion = signal<string>('');
  readonly typedHeroLine = signal<string>('Latest iPhone on monthly part payment.');
  ...
  private readonly typingPhrases = [
    'Latest iPhone on monthly part payment.',
    'Latest laptop and gaming console with easy EMI.',
    'No credit card needed to apply.',
    'Student age 18 plus can apply.',
    'Low income is okay if your profile is clean.',
    'No cost EMI up to 12 months.',
    'If you have 3 or more active loans, not eligible.',
    'Get top camera iPhone without full payment today.'
  ];
  ...
  constructor(
    ...
    private demoMusicService: DemoMusicService
  ) { }
  ...
  ngOnInit(): void {
    ...
    effect(() => {
      ...
      const musicReady = this.demoMusicService.musicReady();
      if (!active || active.soundEnabled !== false || !musicReady || this.loading()) {
        return;
      }
      if (!this.isPlaying() || this.autoplayBlocked()) {
        return;
      }
      window.setTimeout(() => this.tryPlayActive('music_ready'), 20);
    });
  }
  ...
  isFallbackMusicLoading(): boolean {
    const current = this.activeVideo();
    if (!current || current.soundEnabled !== false) {
      return false;
    }
    return !this.demoMusicService.musicReady();
  }

  onVideoLoadStart(): void {
    this.activeVideoLoadPercent.set(1);
  }
  ...
  onVideoProgress(): void {
    ...
    this.activeVideoLoadPercent.set(this.estimateLoadPercent(video));
  }

  onVideoCanPlay(): void {
    const current = this.activeVideoLoadPercent();
    this.activeVideoLoadPercent.set(Math.max(current, 85));
  }

  onVideoLoaded(): void {
    this.activeVideoLoadPercent.set(100);
  ...
  toggleAudio(): void {
    ...
    const video = this.activeVideoRef?.nativeElement;
    if (video) {
      const usesDemoMusic = this.activeVideo()?.soundEnabled === false;
      video.muted = usesDemoMusic ? true : this.isMuted();
      if (!video.paused) {
        this.tryPlayActive('audio_toggle', true);
      }
    }
    this.syncAudioForActive();
  }
  ...
  private fetchManifest(): void {
    ...
    this.manifestService.getManifest('hero', device).subscribe((payload) => {
      ...
      this.isMuted.set(items[0]?.soundEnabled === false ? false : true);
  ...
  private setActiveIndex(nextIndex: number, reason: string): void {
    ...
    this.autoplayBlocked.set(false);
    this.activeVideoLoadPercent.set(1);
    this.isMuted.set(this.videos()[normalized]?.soundEnabled === false ? false : true);
    this.demoMusicService.stopAll();
  ...
  private tryPlayActive(reason: string, fromGesture = false): void {
    ...
    const useDemoMusic = this.activeVideo()?.soundEnabled === false;
    if (useDemoMusic && !this.demoMusicService.musicReady()) {
      this.demoMusicService.prime();
      video.pause();
      return;
    }
    video.muted = useDemoMusic ? true : this.isMuted();
  ...
  private startTypingAnimation(): void {
    if (!this.browserReady || !this.typingPhrases.length) {
      return;
    }
    ...
    this.typedHeroLine.set('');

    const tick = () => {
      const phrase = this.typingPhrases[this.typingPhraseIndex] || '';
      if (!phrase) {
        this.typingTimer = window.setTimeout(tick, 400);
        return;
      }
      ...
      this.typedHeroLine.set(phrase.slice(0, this.typingCharIndex));

      let nextDelay = this.typingDeleting ? 34 : 52;
      ...
      if (!this.typingDeleting && this.typingCharIndex >= phrase.length) {
        ...
        this.typingDeleting = false;
        this.typingPhraseIndex = (this.typingPhraseIndex + 1) % this.typingPhrases.length;
        nextDelay = 240;
      }

      this.typingTimer = window.setTimeout(tick, nextDelay);
    };
  ...
  private syncAudioForActive(): void {
    const active = this.activeVideo();
    if (!active) {
      this.demoMusicService.stopAll();
      return;
    ...
    const video = this.activeVideoRef?.nativeElement;
    if (!video || video.paused || !this.isPlaying() || this.autoplayBlocked()) {
      this.demoMusicService.stop(`hero:${active.id}`);
      return;
    }

    if (active.soundEnabled === false) {
      if (!this.demoMusicService.musicReady()) {
        this.demoMusicService.prime();
        this.demoMusicService.stop(`hero:${active.id}`);
        return;
      }
      ...
      if (this.isMuted()) {
        ...
        this.demoMusicService.start(`hero:${active.id}`);
      }
      return;
    }
  ...
  private estimateLoadPercent(video: HTMLVideoElement): number {
    try {
      ...
      if (Number.isFinite(duration) && duration > 0 && video.buffered.length > 0) {
        ...
        const fromBuffered = Math.round((maxBuffered / duration) * 100);
        if (Number.isFinite(fromBuffered) && fromBuffered > 0) {
          return Math.max(1, Math.min(100, fromBuffered));
        }
      }
    } catch {
      // Ignore buffered range access errors and fall back to readyState heuristics.
    }
