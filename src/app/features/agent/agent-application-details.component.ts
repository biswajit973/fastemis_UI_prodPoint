...
import { AgreementDocumentViewComponent } from '../agreement/components/agreement-document-view.component';

@Component({
  selector: 'app-agent-application-details',
  standalone: true,
  imports: [CommonModule, RouterLink, AgreementDocumentViewComponent],
  changeDetection: ChangeDetectionStrategy.OnPush,
  template: `
    <div class="px-3 sm:px-5 lg:px-8 max-w-6xl mx-auto pb-10">
      <a routerLink="/agent/applicants" class="inline-flex items-center gap-2 text-sm text-secondary no-underline hover:text-primary my-4">
      ...
      <ng-container *ngIf="!loading() && user(); else emptyState">
        ...
        <div class="mb-4 flex gap-2">
          ...
          <button type="button" (click)="setTab('agreement')" class="px-4 py-2 rounded-lg border text-sm"
            [ngClass]="activeTab() === 'agreement' ? 'bg-primary text-white border-primary' : 'border-border text-secondary hover:text-primary'">
            Agreement Document
          </button>
            ...
            <div *ngFor="let field of user()!.field_statuses; trackBy: trackByFieldKey" class="px-4 py-3">
              ...
              <ng-container *ngIf="isPreviewableMedia(field); else textValueMobile">
                <div class="rounded-lg border border-border bg-surface p-2">
                  <img
                    *ngIf="mediaType(field.value) === 'image' && !hasMediaLoadError(field.key)"
                    [src]="mediaUrl(field.value)"
                    alt="Uploaded media"
                    loading="lazy"
                    decoding="async"
                    (load)="clearMediaLoadError(field.key)"
                    (error)="markMediaLoadError(field.key)"
                    class="w-full max-h-48 object-contain rounded bg-surface-2" />
                  <div
                    *ngIf="mediaType(field.value) === 'image' && hasMediaLoadError(field.key)"
                    class="rounded-lg border border-warning/40 bg-warning/10 px-3 py-2 text-xs text-warning">
                    {{ mediaPreviewErrorMessage(field) }}
                  </div>
                  <video
        ...
        <section *ngIf="activeTab() === 'agreement'" class="space-y-4">
          <div *ngIf="agreementDocumentLoading()" class="rounded-xl border border-border bg-surface p-10 flex items-center justify-center">
            <div class="w-8 h-8 rounded-full border-2 border-surface-3 border-t-primary animate-spin"></div>
          </div>

          <div *ngIf="!agreementDocumentLoading() && !agreementDocument()" class="rounded-xl border border-border bg-surface p-6 text-center text-secondary">
            Agreement document is not available for this user yet.
          </div>

          <app-agreement-document-view
            *ngIf="!agreementDocumentLoading() && agreementDocument()"
            [document]="agreementDocument()"
            heading="Customer Agreement Record"
            subheading="This is the final signed record seen by the customer, including uploaded evidence and execution metadata."
      ...
      <div *ngIf="previewField()" class="fixed inset-0 z-[95] bg-black/85 p-4 flex flex-col">
        ...
        <div class="flex-1 min-h-0 rounded-xl border border-white/20 bg-black/30 p-3 flex items-center justify-center overflow-auto">
          <img
            *ngIf="previewField() && mediaType(previewField()!.value) === 'image' && !hasMediaLoadError(previewField()!.key)"
            [src]="mediaUrl(previewField()!.value)"
            alt="Media preview"
            (load)="clearMediaLoadError(previewField()!.key)"
            (error)="markMediaLoadError(previewField()!.key)"
            class="max-h-full max-w-full object-contain rounded" />
          <div
            *ngIf="previewField() && mediaType(previewField()!.value) === 'image' && hasMediaLoadError(previewField()!.key)"
            class="text-center text-white">
            <p class="text-sm mb-3">{{ mediaPreviewErrorMessage(previewField()!) }}</p>
            <a
...
export class AgentApplicationDetailsComponent implements OnInit {
  ...
  readonly activeTab = signal<'profile' | 'agreement' | 'management'>('profile');
  readonly previewField = signal<AgentFieldStatus | null>(null);
  readonly mediaLoadErrors = signal<Record<string, boolean>>({});
  readonly agreementDocumentLoading = signal<boolean>(false);
  ...
  ngOnInit(): void {
    ...
    } else if (queryTab === 'agreement') {
      this.activeTab.set('agreement');
    }

    this.loadDetails();
    if (this.activeTab() === 'agreement') {
      this.loadAgreementDocument();
    }
  }

  setTab(tab: 'profile' | 'agreement' | 'management'): void {
  ...
  deleteUser(): void {
    ...
    this.agentUsersApi.deleteUser(current.id).subscribe((ok) => {
      ...
      this.router.navigate(['/agent/applicants']);
    });
  }
  ...
  hasMediaLoadError(fieldKey: string): boolean {
    return !!this.mediaLoadErrors()[fieldKey];
  }

  markMediaLoadError(fieldKey: string): void {
    this.mediaLoadErrors.update((current) => ({ ...current, [fieldKey]: true }));
  }

  clearMediaLoadError(fieldKey: string): void {
    if (!this.mediaLoadErrors()[fieldKey]) {
      return;
    }

    this.mediaLoadErrors.update((current) => {
      const next = { ...current };
  ...
  mediaPreviewErrorMessage(field: AgentFieldStatus): string {
    if (field.key === 'agreement_signature') {
      return 'Signature file is invalid or preview is unavailable.';
    }
    return 'Preview is unavailable for this uploaded file.';
  }
  ...
  private loadAgreementDocument(): void {
    if (!this.userId || this.agreementDocumentLoading()) {
      return;
    }

    this.agreementDocumentLoading.set(true);
    this.agreementApi.getAgentAgreementDocument(this.userId).subscribe((documentPayload) => {
      this.agreementDocument.set(documentPayload);
      this.agreementDocumentLoading.set(false);
    });
  }
