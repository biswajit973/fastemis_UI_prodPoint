    ...
    <div class="max-w-7xl mx-auto px-2 sm:px-4 pb-8 space-y-6">
      ...
      <section class="rounded-2xl border border-border bg-surface p-4 sm:p-6 shadow-sm">
        ...
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-4">
          ...
          <div class="flex items-end">
            <label class="inline-flex items-center gap-2 text-sm text-primary font-medium cursor-pointer">
              <input [(ngModel)]="newSoundEnabled" type="checkbox" class="h-4 w-4 rounded border-border text-primary">
              Allow Original Sound
            </label>
          </div>
        ...
        <div class="mt-4 flex flex-wrap items-center gap-3">
          <button
            ...
            {{ isCreateBusy() ? 'Uploading...' : 'Upload Video' }}
          </button>
        ...
        <div *ngIf="isCreateBusy()" class="mt-4 rounded-xl border border-border bg-surface-2 p-3">
          <div class="flex items-center justify-between gap-3 text-xs font-semibold text-secondary">
        ...
        <div *ngIf="!loading()" class="divide-y divide-border">
          <article
            *ngFor="let video of filteredVideos(); trackBy: trackByVideo"
            class="p-4 sm:p-5 transition-opacity duration-200"
            [class.opacity-75]="isVideoBusy(video)">
              ...
              <div class="space-y-3">
                <div class="flex flex-wrap items-start justify-between gap-3">
                  ...
                  <div class="flex flex-wrap items-center gap-2">
                    ...
                    <span class="rounded-full px-2.5 py-1 text-xs font-semibold border"
                      [ngClass]="video.sound_enabled ? 'bg-emerald-50 text-emerald-700 border-emerald-200' : 'bg-amber-50 text-amber-700 border-amber-200'">
                      {{ video.sound_enabled ? 'Sound: On' : 'Sound: Off' }}
                    </span>
                  </div>
                </div>
                ...
                <div class="flex flex-wrap items-center gap-2 pt-1">
                  <button
                    ...
                    {{ isVideoBusy(video) ? 'Saving...' : (video.is_active ? 'Disable Video' : 'Enable Video') }}
                  </button>
                  ...
                  <button
                    ...
                    {{ isVideoBusy(video) ? 'Saving...' : (video.show_in_hero ? 'Remove from Hero' : 'Add to Hero') }}
                  </button>

                  <button
                    type="button"
                    (click)="toggleSound(video)"
                    [disabled]="actionBusy()"
                    class="rounded-lg px-3 py-2 text-sm font-semibold border border-amber-300 text-amber-700 bg-amber-50 hover:bg-amber-100">
                    {{ isVideoBusy(video) ? 'Saving...' : (video.sound_enabled ? 'Turn Sound Off' : 'Turn Sound On') }}
                  </button>
                </div>

                <p *ngIf="isVideoBusy(video)" class="text-xs font-medium text-secondary">
...
export class AgentVideoManagementComponent implements OnInit {
  ...
  readonly actionContext = this.videoApi.actionContext;
  readonly uploadProgress = this.videoApi.uploadProgress;
  ...
  uploadVideo(): void {
    ...
    this.videoApi.createVideo({
      ...
      soundEnabled: this.newSoundEnabled,
  ...
  toggleSound(video: AgentVideoItem): void {
    if (this.actionBusy()) return;
    this.successMessage.set('');
    this.videoApi.updateVideo(video.id, { sound_enabled: !video.sound_enabled }).subscribe((updated) => {
      if (!updated) return;
      this.successMessage.set(updated.sound_enabled ? 'Original sound enabled for this video.' : 'Original sound disabled. Demo music will be used.');
    });
  }

  isCreateBusy(): boolean {
    return this.actionBusy() && this.actionContext() === 'create';
  }

  isVideoBusy(video: AgentVideoItem): boolean {
    return this.actionBusy() && this.actionContext() === `video:${video.id}`;
  }
