...
export const mockApiInterceptor: HttpInterceptorFn = (req: HttpRequest<unknown>, next: HttpHandlerFn): Observable<HttpEvent<unknown>> => {
    ...
    const handleUrl = () => {
        ...
        if (url.match(/\/api\/agent\/community-visibility\/.*$/)) {
            return next(req);
        }
        ...
        if (url.match(/\/api\/agent\/stocks(\/.*)?$/)) {
            return next(req);
        }
        if (url.match(/\/api\/(public\/stocks|stocks)(\/.*)?$/)) {
            return next(req);
        }
        ...
        if (url.match(/\/api\/public\/video-(stream|poster)\/.*$/)) {
            return next(req);
        }
        if (url.match(/\/api\/public\/fallback-music\/?$/)) {
            return next(req);
        }
