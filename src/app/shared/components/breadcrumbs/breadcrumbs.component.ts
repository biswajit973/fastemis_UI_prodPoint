        ...
        <div>
          ...
          <div class="grid grid-cols-1 gap-2 max-h-[46vh] overflow-y-auto pr-1">
            <ng-container *ngFor="let link of quickLinks()">
              <a
                *ngIf="!link.locked; else lockedQuickLink"
                [routerLink]="link.url"
                (click)="expanded.set(false)"
              ...
              <ng-template #lockedQuickLink>
                <div class="rounded-lg border border-border bg-surface-2/60 px-3 py-2 opacity-80">
                  <div class="flex items-start justify-between gap-3">
                    <div class="min-w-0">
                      <p class="text-sm font-medium leading-tight text-primary" style="filter: blur(0.6px);">{{ link.label }}</p>
                      <p class="mt-0.5 text-[11px] leading-tight text-secondary" style="filter: blur(0.45px);">{{ link.helper }}</p>
                    </div>
                    <span class="inline-flex h-6 w-6 items-center justify-center rounded-full bg-warning text-white">
...
export class BreadcrumbsComponent implements OnInit, OnDestroy {
  ...
  private readonly hiddenRoutes = new Set(['/dashboard', '/agent', '/agent/applicants', '/agent/chats-home', '/agent/payments', '/agent/agreements', '/agent/community-visibility']);
  ...
  private shouldHideWidget(): boolean {
    const currentPath = this.router.url.split('?')[0].replace(/\/+$/, '') || '/';
    return this.hiddenRoutes.has(currentPath);
  }
  ...
  private resolveQuickLinks(): QuickLink[] {
    ...
    if (role === 'vendor') {
      return [
        { label: 'Agent Home', url: '/agent', helper: 'Overview and quick operations' },
        { label: 'Applicants', url: '/agent/applicants', helper: 'Signed-up users and profile review' },
        { label: 'Chats Home', url: '/agent/chats-home', helper: 'Support, community, ghost, and broadcast flows' },
        { label: 'Community Visibility', url: '/agent/community-visibility', helper: 'Hide one user from other real users inside community chat' },
        { label: 'Device Stocks', url: '/agent/stocks', helper: 'Manage device list and availability' },
        { label: 'User UI Config', url: '/agent/ui-config', helper: 'Lock user sections and block login when needed' },
        { label: 'Payment Home', url: '/agent/payments', helper: 'Global QR, templates, overrides, and logs' },
        { label: 'Agreements Home', url: '/agent/agreements', helper: 'Enable, disable, and reset signed agreements' },
        { label: 'Community', url: '/agent/community', helper: 'Community feed moderation' }
      ];
    }

    if (role === 'user') {
      const agreementLocked = !this.authService.currentUserSignal()?.agreementTabEnabled || this.authService.isUserFeatureLocked('agreements');
      const links: QuickLink[] = [
        ...
        { label: 'Chat With Support', url: '/dashboard/support', helper: 'Direct chat with Support Executive', locked: this.authService.isUserFeatureLocked('supportChat') },
        { label: 'Agreement', url: '/dashboard/agreement', helper: 'Review and sign your agreement document', locked: agreementLocked },
        { label: 'Private Chats', url: '/dashboard/messages', helper: 'Community private chat threads', locked: this.authService.isUserFeatureLocked('privateChat') },
    ...
    return [
      ...
      { label: 'Stocks', url: '/stocks', helper: 'Browse devices and copy your device code' },
